            ...
            <div className="flex items-center space-x-8">
              {navLinks.map((link) => (
                <Link key={link.path} href={link.path}>
                  <a className={`nav-link text-white hover:text-accent font-medium ${isActive(link.path)}`} onClick={closeMenu}>
                    {link.name}
                  </a>
                </Link>
              ))}
          ...
          >
            <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
              {navLinks.map((link) => (
                <Link key={link.path} href={link.path}>
                  <a
                    className="block px-3 py-2 rounded-md text-white hover:bg-primary/20 hover:text-accent"
                    onClick={closeMenu}
                  >
                    {link.name}
                  </a>
              ...
              <Link href="/trader-portal">
                <a
                  className="block px-3 py-2 rounded-md text-accent hover:bg-primary/20"
                  onClick={closeMenu}
                >
                  Trader Portal
                </a>
