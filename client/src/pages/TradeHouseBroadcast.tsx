...
import { decodeQuickRoster, useTradeHouseFeed, type Standing, type LeaderboardPayload } from "@/lib/tradehouse-feed";
...
const TraderPlate: React.FC<{ trader: Standing; side?: "left" | "right"; large?: boolean }> = ({ trader, side = "left", large = false }) => {
  ...
  return (
    ...
    >
      <div className={`absolute inset-y-0 ${side === "left" ? "left-0" : "right-0"} w-1.5 bg-gradient-to-b from-cyan-300 via-emerald-300 to-cyan-300`} />
  ...
  return (
    <div className="flex h-full w-full items-start justify-end p-10">
      <div className="w-[560px] overflow-hidden rounded-[28px] border border-white/15 bg-[#070b13]/92 backdrop-blur-2xl" style={{ boxShadow: "0 30px 100px rgba(0,0,0,.5)" }}>
        <div className="flex items-center justify-between border-b border-white/10 bg-gradient-to-r from-cyan-300/10 via-white/[0.02] to-emerald-300/10 px-6 py-5">
          <div>
            <div className="font-['Orbitron'] text-[10px] font-black uppercase tracking-[0.22em] text-cyan-300">Trade House</div>
        ...
        <div>
          {top.map((trader) => (
            <div key={trader.id} className="grid grid-cols-[58px_1fr_135px] items-center gap-3 border-b border-white/[0.06] px-6 py-4 last:border-0">
              <div className={`font-['Orbitron'] text-xl font-black ${trader.rank <= 3 ? "text-cyan-300" : "text-slate-400"}`}>#{trader.rank}</div>
  ...
  return (
    <div className="relative h-full w-full">
      ...
      <div className="absolute bottom-[80px] left-1/2 -translate-x-1/2">
        <div className="flex h-16 w-16 items-center justify-center rounded-full border border-cyan-300/30 bg-[#070b13]/95 font-['Orbitron'] text-lg font-black text-cyan-300 shadow-[0_0_40px_rgba(34,211,238,.22)]">
          VS
        </div>
      </div>
    </div>
  ...
  return (
    <div className="flex h-full w-full items-start justify-center p-7">
      <div className="flex items-center gap-5 rounded-2xl border border-white/15 bg-[#070b13]/92 px-5 py-3.5 backdrop-blur-2xl" style={{ boxShadow: "0 16px 55px rgba(0,0,0,.4)" }}>
        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-violet-400/10"><Crown className="h-5 w-5 text-cyan-300" /></div>
        <div>
...
const TradeHouseBroadcast: React.FC = () => {
  ...
  const quickRoster = useMemo(() => decodeQuickRoster(quickKey), [quickKey]);
  const { data, state } = useTradeHouseFeed(qs.get("demo") === "1", quickRoster, qs.get("season") || "Quick Battle");
