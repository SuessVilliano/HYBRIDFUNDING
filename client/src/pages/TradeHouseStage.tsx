    ...
    <div className={`th-stage ${overlay?'th-overlay':''}`} style={{transform:`scale(${scale})`,left:(size.w-1920*scale)/2,top:(size.h-1080*scale)/2}}>
      ...
      <section className={`th-floor ${layout==='grid'?'th-grid':'th-duel'}`}>
        {selected.map((trader,i)=><article key={trader?.id||i} className={`th-seat th-seat-${i%2}`}>
          ...
          <div className="th-plate"><div className="th-trader-name">{trader?.avatarUrl&&<img src={trader.avatarUrl} alt="" style={{width:34,height:34,borderRadius:8,objectFit:'cover',marginRight:10,verticalAlign:'middle'}}/>}{trader?.name||'Open seat'}<small>{demo?'SAMPLE CONTESTANT':valid(trader)?`${(trader?.division||'trading').toUpperCase()} · ${(trader?.platform||'other').toUpperCase()} · VERIFIED`:trader?'DATA UNAVAILABLE':'CONTESTANT NOT ASSIGNED'}</small></div><div className={`th-pnl ${valid(trader)&&trader!.pnl<0?'th-loss':''}`}>{valid(trader)?cash(trader!.pnl):'—'}<small>{valid(trader)?`${trader!.returnPct>=0?'+':''}${trader!.returnPct.toFixed(2)}% RETURN`:'AWAITING VERIFIED DATA'}</small></div></div>
