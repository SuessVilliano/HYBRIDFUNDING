...
function Board({ traders }: { traders: Standing[] }) {
  return <div className="tv-board"><div className="tv-board-head"><div><small>TRADE HOUSE</small><h1>THE HOUSE STANDINGS</h1></div><div className="tv-board-live"><Radio size={16} /> UPDATING EVERY 15 SEC</div></div><div className="tv-board-grid">{traders.slice(0, 8).map((t) => <div className="tv-board-row" key={t.id}><strong>#{t.rank}</strong><div className="tv-board-name">{t.avatarUrl&&<img src={t.avatarUrl} alt="" style={{width:28,height:28,borderRadius:7,objectFit:"cover",marginRight:8}}/>}<span>{t.name}</span>{t.verified && <ShieldCheck size={14} /> }<small>{(t.division || "trading").toUpperCase()} · {(t.platform || "other").toUpperCase()} · {t.status === "live" ? "POSITION OPEN" : t.status.toUpperCase()} · {t.tradeCount} TRADES</small></div><div className={t.pnl >= 0 ? "tv-positive" : "tv-negative"}>{t.pnl >= 0 ? "+" : ""}{money(t.pnl)}<small>{t.returnPct >= 0 ? "+" : ""}{t.returnPct.toFixed(2)}%</small></div></div>)}</div></div>;
}
function Duel({ left, right }: { left?: Standing; right?: Standing }) {
  return <div className="tv-duel"><div className="tv-duel-card tv-cyan"><small>CHALLENGER A · {(left?.division || "trading").toUpperCase()} · RANK #{left?.rank || "—"}</small>{left?.avatarUrl&&<img src={left.avatarUrl} alt="" style={{width:58,height:58,borderRadius:14,objectFit:"cover",marginBottom:12}}/>}<h2>{left?.name || "OPEN SEAT"}</h2><div className={left && left.pnl < 0 ? "tv-negative" : "tv-positive"}>{left ? `${left.pnl >= 0 ? "+" : ""}${money(left.pnl)}` : "—"}<em>{left ? `${left.returnPct >= 0 ? "+" : ""}${left.returnPct.toFixed(2)}% RETURN` : "AWAITING FEED"}</em></div></div><div className="tv-vs">VS</div><div className="tv-duel-card tv-purple"><small>CHALLENGER B · {(right?.division || "trading").toUpperCase()} · RANK #{right?.rank || "—"}</small>{right?.avatarUrl&&<img src={right.avatarUrl} alt="" style={{width:58,height:58,borderRadius:14,objectFit:"cover",marginBottom:12}}/>}<h2>{right?.name || "OPEN SEAT"}</h2><div className={right && right.pnl < 0 ? "tv-negative" : "tv-positive"}>{right ? `${right.pnl >= 0 ? "+" : ""}${money(right.pnl)}` : "—"}<em>{right ? `${right.returnPct >= 0 ? "+" : ""}${right.returnPct.toFixed(2)}% RETURN` : "AWAITING FEED"}</em></div></div></div>;
}
